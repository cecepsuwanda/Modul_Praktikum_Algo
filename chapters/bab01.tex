...
\end{itemize}
\end{document}
% Toolchain resmi untuk buku ini
\subsection{Toolchain Resmi: OnlineGDB, Lazarus, Code::Blocks}
Untuk konsistensi praktikum:
\begin{itemize}
  \item \textbf{Online (semua bahasa)}: OnlineGDB — jalankan Pascal, C, C++ langsung di browser \parencite{onlinegdb}.
  \item \textbf{Offline Pascal}: Lazarus IDE (berbasis Free Pascal) \parencite{lazarus}.
  \item \textbf{Offline C/C++}: Code::Blocks IDE (dengan toolchain MinGW/Clang sesuai OS) \parencite{codeblocks}.
\end{itemize}

Langkah cepat:
\begin{enumerate}
  \item \textbf{OnlineGDB}: buka proyek baru, pilih bahasa (Pascal/C/C++), tempel kode, klik Run.
  \item \textbf{Lazarus}: buat Project \textrightarrow{} Program; tambahkan unit bila perlu; Build \& Run.
  \item \textbf{Code::Blocks}: buat Console Application (C atau C++), letakkan sumber di \texttt{src/}, Build \& Run.
\end{enumerate}

\begin{table}[h]
  \centering
  \caption{Ringkasan toolchain praktikum}
  \begin{tabular}{@{}lll@{}}
    \toprule
    Modus & Pascal & C/C++ \\
    \midrule
    Online & OnlineGDB & OnlineGDB \\
    Offline & Lazarus & Code::Blocks \\
    \bottomrule
  \end{tabular}
\end{table}
